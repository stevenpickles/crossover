...
- **The layout editor draws your screens in their real proportions.** A
  ...
  remote display, a panel with no readable EDID) are drawn from their pixel
  counts, scaled to sit believably beside the screens that could be
  measured, and captioned `(size estimated)` so it is clear which
  rectangles are guesses. Only *new* rectangles are sized this way: an
...
- **Monitors report how big they physically are.** Each machine now reads
  ...
  drawing (above) is built on. A screen whose size cannot be read
